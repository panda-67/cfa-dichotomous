...
generate_manual_model <- function() {
  paste(
    "C1 =~",
    paste(items_to_lavaan(C1_items), collapse = " + "),
    "C2 =~",
    paste(items_to_lavaan(C2_items), collapse = " + "),
    "C3 =~",
    paste(items_to_lavaan(C3_items), collapse = " + "),
    "C4 =~",
    paste(items_to_lavaan(C4_items), collapse = " + "),
    sep = "\n"
  )
}
...
generate_efa_model <- function(items, thresh = .30, nfactors = 4) {
  ...
  items_num <- as.data.frame(lapply(items, function(x) {
...
load_or_generate_model <- function(
  items,
  efa = TRUE,
  thresh = 0.30,
  file = NULL,
  ...
  if (efa) {
    ...
    if (!file.exists(file)) {
      if (VERBOSE) {
        cat(">> Manual file missing → creating EFA-guided template...\n")
      }
  ...
  return(generate_initial_model(items, efa_override = efa, thresh = thresh))
}
